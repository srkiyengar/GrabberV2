...
MAX_SPEED = 300 # A max speed of 1023 is allowed
...
LOG_FILENAME = 'Grabberv2' + datetime.now().strftime('%Y-%m-%d---%H:%M:%S')
            ...
            if event.type == pygame.QUIT:
                ...
                if Hat[0] == -1:
                    if my_dataset.empty:
                        ...
                        one_datafile.write_data_file("Total transit time is +ive if Desktop is ahead: {} microsecond\n".format(
                        ...
                        if my_cam.cam_detected == 1:
                            my_cam.capture_and_save_frame(finger_file)
                        my_connector.start_collecting(finger_file)
                if Hat[0] == 1:
                    if set_record == 1:
                        F = palm.get_palm_current_position()
                        ...
                        pass
                if Hat[1] == -1:
                    finger_speed = palm.get_servo_speed()
                    my_logger.info("Current Speed: {}\n".format(finger_speed))
                    finger_speed = finger_speed + 50
                    palm.set_servo_speed(finger_speed)
                    my_logger.info("New Speed: {}\n".format(finger_speed))
                if Hat[1] == 1:
                    finger_speed = palm.get_servo_speed()
                    my_logger.info("Current Speed: {}\n".format(finger_speed))
                    finger_speed = finger_speed - 50
...
if old_datafile:        #when no batch is created, the program won't try to close a non-existant file handle
    one_datafile.close_data_file()
my_connector.destroy()    #close socket
